...
class Controller:
    ...
    def changesystembrightness():
        brightness_info = sbcontrol.get_brightness()

        try:
            # Attempt to extract the brightness value from the list
            brightness_value = brightness_info[0]
        except (TypeError, IndexError):
            # Handle the case where the list is empty or index out of range
            brightness_value = 0

        try:
            # Attempt to convert the brightness value to a float
            currentBrightnessLv = float(brightness_value) / 100.0
        except (ValueError, TypeError):
            # Handle the case where the conversion fails
            currentBrightnessLv = brightness_value[1:-1] / 100.0
        ...
        sbcontrol.fade_brightness(
            int(100 * currentBrightnessLv), start=brightness_value
        )
...
class GestureController:
    ...
    def __init__(self):
        ...
        GestureController.cap = cv2.VideoCapture(0)  # Use default camera
